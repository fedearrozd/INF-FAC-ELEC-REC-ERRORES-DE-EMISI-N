...
GcProgEmail_Tipo= 'ERREMIMIN'	&& Todos
...
IF GlExis
	...
	SELECT FConfSQL
	LcAplicacion= 'GERENCIAL'
	SET FILTER TO ALLTRIM(Aplicacion)= LcAplicacion AND ALLTRIM(Tipo)= 'CONEXINT'
	GO TOP
	LcBDGeren= ALLTRIM(BaseDatos)
					...
					IF !GlCancelar AND LnCntCorreos> 0
						...
						IF LnCntEnvia > 0
							...
							DO WHILE !EOF()
								...
								LcPerfil= ""
								LcPassword= ""
								LlFExpoMailW = FExpoMailW('GTPlCorreo',LcCorreo,LcCod_Acceso,LcAsunto,GcDirectorio,LnPuerto)
								SELECT GTPlCorreo
								REPLACE FecIng WITH DATE()
								REPLACE PasswordIn WITH GcUser
								IF !LlFExpoMailW
									REPLACE RES_ESTADO WITH .F.
									REPLACE Enviado WITH .F.
									REPLACE Nume_Err WITH GmAError[1,1]
									REPLACE text_Err WITH GmAError[1,2]
									REPLACE PARA_ERR WITH GmAError[1,3]
									REPLACE area_Err WITH GmAError[1,4]
									REPLACE codi_Err WITH GmAError[1,5]
									REPLACE ayud_Err WITH GmAError[1,6]
									REPLACE exce_Err WITH GmAError[1,7]
									RELEASE GmAError
								ELSE
									REPLACE RES_ESTADO WITH .T.
